...
import com.xyk.R;
import com.xyk.Utils.DimenUtil;
...
public class CircularProgressView extends View {
    ...
    private Paint mBackPaint, mProgPaint, mText;   // 绘制画笔
...
//        初始化字体
        mText = new Paint();
        textColor = typedArray.getColor(R.styleable.CircularProgressView_TabtextColor, ContextCompat.getColor(getContext(), R.color.black));
        textSize = typedArray.getDimension(R.styleable.CircularProgressView_TabtextSize, DimenUtil.dp2px(getContext(), 14.0f));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //第三步:画圆环内百分比文字
        float textWidth = mText.measureText(mProgress + "%"); //测量字体宽度，我们需要根据字体的宽度设置在圆环中间
        canvas.drawText(mProgress + "%", centre - textWidth / 2, centre + textSize / 2, mText); //画出进度百分比
    }
    ...
    public void setTextColor(@ColorRes int color) {
        mText.setColor(ContextCompat.getColor(getContext(), color));
        invalidate();
